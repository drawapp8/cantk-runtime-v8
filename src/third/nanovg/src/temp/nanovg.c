...
void nvgTransformPointWithXForm(NVGcontext* ctx, float x, float y, float* ox, float* oy) {
	NVGstate* state = nvg__getState(ctx);
	nvgTransformPoint(ox, oy, state->xform, x, y);
...
// Scissoring
void nvgScissor(NVGcontext* ctx, int x, int y, int w, int h)
{
	ctx->params.renderScissor(ctx->params.userPtr, x, y, w, h);
}

void nvgResetScissor(NVGcontext* ctx)
{
	ctx->params.renderScissor(ctx->params.userPtr, 0, 0, 0, 0);
}
...
void nvgIntersectScissor(NVGcontext* ctx, int x, int y, int w, int h)
{
	NVGstate* state = nvg__getState(ctx);
...
void nvgFill(NVGcontext* ctx)
{
	...
	nvgTransformIdentity(fillPaint.xform);
	nvgTransformMultiply(fillPaint.xform, state->xform);
...
void nvgStroke(NVGcontext* ctx)
{
	...
	nvgTransformIdentity(strokePaint.xform);
	nvgTransformMultiply(strokePaint.xform, state->xform);
